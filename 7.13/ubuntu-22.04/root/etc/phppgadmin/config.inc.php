<?php
        ...
        $conf['servers'][0]['desc'] = getenv('DATABASE_DESC')== false ? 'PostgreSQL' : getenv('DATABASE_DESC');

        // Hostname or IP address for server.  Use '' for UNIX domain socket.
        // use 'localhost' for TCP/IP connection on this computer
        $conf['servers'][0]['host'] = getenv('DATABASE_HOST')== false ? 'localhost' : getenv('DATABASE_HOST');

        // Database port on server (5432 is the PostgreSQL default)
        $conf['servers'][0]['port'] = getenv('DATABASE_PORT')== false ? 5432 : getenv('DATABASE_PORT');

        // Database SSL mode
        // Possible options: disable, allow, prefer, require
        // To require SSL on older servers use option: legacy
        // To ignore the SSL mode, use option: unspecified
        $conf['servers'][0]['sslmode'] = getenv('DATABASE_SSLMODE')== false ? 'allow' : getenv('DATABASE_PORT');
